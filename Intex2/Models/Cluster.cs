using System;
using System.Collections.Generic;

#nullable disable

namespace Intex2.Models
{
    public partial class Cluster
    {
        public double? BurialId2018 { get; set; }
        public double? Burialy { get; set; }
        public string Burialnors { get; set; }
        public double? Burialx { get; set; }
        public string Burialxeorw { get; set; }
        public string Burialplotquad { get; set; }
        public double? Burialextractorder { get; set; }
        public double? Burialxtohead { get; set; }
        public double? Burialxtofeet { get; set; }
        public double? Burialytohead { get; set; }
        public double? Burialytofeet { get; set; }
        public double? Burialz { get; set; }
        public double? Length { get; set; }
        public double? Yearexcav { get; set; }
        public string Burialdirection { get; set; }
        public double? Burialpreservation { get; set; }
        public string Burialwrapping { get; set; }
        public string Burialadultchild { get; set; }
        public string GenderCode { get; set; }
        public string Burialgendermethod { get; set; }
        public string AgeCodeSingle { get; set; }
        public string BurialDirection1 { get; set; }
        public string Burialageatdeath { get; set; }
        public string Burialagemethod { get; set; }
        public string HairColorCode { get; set; }
        public string Burialhaircolor { get; set; }
        public bool? Burialsampletaken { get; set; }
        public double? LengthM { get; set; }
        public double? LengthCm { get; set; }
        public double? Jewelry { get; set; }
        public double? Cluster1 { get; set; }
        public string F32 { get; set; }
        public string F33 { get; set; }
        public string F34 { get; set; }
        public string F35 { get; set; }
        public double? F36 { get; set; }
        public double? F37 { get; set; }
        public double? F38 { get; set; }
        public double? F39 { get; set; }
        public double? F40 { get; set; }
        public double? F41 { get; set; }
        public double? F42 { get; set; }
        public double? F43 { get; set; }
        public int Id { get; set; }
        public byte[] SsmaTimeStamp { get; set; }
    }
}
